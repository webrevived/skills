...
| ----- | -------- | ------------ | ------ |
| [codex-loop](./skills/engineering/codex-loop/SKILL.md) | Engineering | Reviews staged changes with Astra using recall-biased finders and skeptic verifiers at pinned high effort, fixes findings, then verifies; use `--model sol` for Sol. | Claude Code/Cursor: `/codex-loop`; Codex: `$codex-loop` |
